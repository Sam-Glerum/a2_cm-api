...
// TEST input
const TEST_MERCHANTCHECK_ID = '0094cc359258ae2a28d545b123';

const TEST_USER_NAME = 'tester';
const TEST_USER_PASS = 'soepersiekret';
let token;

xdescribe('MerchantCheck:', () => {
    ...
    it('GET: \nShould return ONE merchantCheck when performing a GET with specific ID', (done) => {
        chai.request(server)
            .get('/api/merchantchecks/' + TEST_MERCHANTCHECK_ID)
            .set('X-Access-Token', token)
            .end((err, res) => {
                res.body.merchantCheck.should.have.lengthOf(1);
